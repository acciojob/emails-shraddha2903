...
public class Workspace extends Gmail{
    ...
    public int findMaxMeetings(){
        ...
        List<Meeting> sortedCalender = new ArrayList(List.copyOf(this.calendar));
        Collections.sort(sortedCalender, (x, y) -> {
            return x.getEndTime().compareTo(y.getEndTime());
        });
        List<Meeting> maxMeeting = new ArrayList();
        LocalTime time_limit = ((Meeting)sortedCalender.get(0)).getEndTime();
        maxMeeting.add((Meeting)sortedCalender.get(0));
        Iterator var4 = sortedCalender.iterator();
